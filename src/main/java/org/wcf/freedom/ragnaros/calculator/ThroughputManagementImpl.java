...
import org.wcf.freedom.ragnaros.vo.ThroughputData;
...
public class ThroughputManagementImpl implements ManagementService {
     ...
     * 每秒的吞吐量
     */
    private Map<String, AtomicLong> throughputPerSecondMap = new ConcurrentHashMap<>();
     ...
     * 当前处理的事件平均耗时
     */
    private Map<String, AtomicLong> averageCostMap = new ConcurrentHashMap<>();
    /**
     * 最后一次处理的事件耗时
     */
        ...
        map.forEach((k, v) -> {
            ...
            for (Method method : methods) {
                ...
                if (function != null) {
                    ...
                    throughputPerSecondMap.put(function.name(), new AtomicLong(0L));
                    costWindowMap.put(function.name(), new AtomicLong(0L));
                    averageCostMap.put(function.name(), new AtomicLong(0L));
        ...
        executor.execute(() -> {
            ...
            for (; working; ) {
                ...
                for (String item : totalThroughputMap.keySet()) {
                    ...
                    long t = totalThroughputMap.get(item).get() - lastThroughputMap.get(item).get();
                    throughputPerSecondMap.get(item).set(t / RagnarosConsts.DEFAULT_UNIT_TIME);
                    averageCostMap.get(item).set(t == 0 ? costWindowMap.get(item).get() : (costWindowMap.get(item).get() / t));
    ...
    public ServiceThroughput watch() {
        ...
        for (String item : totalThroughputMap.keySet()) {
            ...
            single.setAverageCost(averageCostMap.get(item).get());
            single.setItemName(item);
            single.setLastCost(lastCostMap.get(item).get());
            single.setThroughput(throughputPerSecondMap.get(item).get());
            list.add(single);
        }
    ...
    private void resetData() {
        ...
        throughputPerSecondMap.forEach((k, v) -> v.set(0L));
        costWindowMap.forEach((k, v) -> v.set(0L));
        averageCostMap.forEach((k, v) -> v.set(0L));
